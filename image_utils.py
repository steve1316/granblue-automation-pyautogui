            ...
            while (summon_location == None and summon_index <= len(summon_list)):
                ...
                if (summon_location == None):
                    ...
                    if(summon_location == None):        
                        ...
                        if(((self._game.farming_mode.lower() != "event" or self._game.farming_mode.lower() != "event (token drawboxes)") and self.find_button("bottom_of_summon_selection", tries=1, suppress_error=True) != None) or 
                           ((self._game.farming_mode.lower() == "event" or self._game.farming_mode.lower() == "event (token drawboxes)") and self.find_button("bottom_of_event_summon_selection", tries=1, suppress_error=True) != None)):
