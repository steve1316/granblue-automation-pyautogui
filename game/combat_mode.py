...
class CombatMode:
    ...
    def _wait_for_attack(self):
        ...
        """
        tries = 10
        while (not self._retreat_check and self.image_tools.find_button("attack", tries = 1, suppress_error = True) is None) or (
                not self._retreat_check and self.image_tools.find_button("next", tries = 1, suppress_error = True) is None):
            ...
            tries -= 1
            if tries < 0 or self.image_tools.find_button("attack", tries = 1, suppress_error = True) is not None or self.image_tools.find_button("next", tries = 1, suppress_error = True) is not None:
                break
    ...
    def start_combat_mode(self, script_file_path: str = "", is_nightmare: bool = False):
        ...
        try:
            ...
            while i < len(lines) and not self._retreat_check and not semi_auto and not full_auto:
                ...
                if "turn" in line.lower() and int(line.split(":")[0].split(" ")[1]) == turn_number and not self._retreat_check:
                    ...
                    while ("end" not in line.lower() and "exit" not in line.lower()) and i < len(lines):
                                    ...
                                    if self.image_tools.confirm_location("use_skill", tries = 1):
                                        ...
                                        if select_a_character_location is not None:
                                            ...
                                            if "target(1)" in target:
                                                ...
                                                self.mouse_tools.move_and_click_point(select_a_character_location[0] - 90,
                                                ...
                                                self.mouse_tools.move_and_click_point(select_a_character_location[0],
                                                                                      select_a_character_location[1] + 85, "template_target")
                        ...
                        if self.image_tools.find_button("next", tries = 1, suppress_error = True) is not None:
                            break
            ...
            while not self._retreat_check and not semi_auto and not full_auto and not self.image_tools.confirm_location("exp_gained", tries = 1) and not self.image_tools.confirm_location("no_loot",
                                                                                                                                                                                           tries = 1):
                self._find_dialog_in_combat()
                attack_button_location = self.image_tools.find_button("attack", tries = 1, suppress_error = True)
                if attack_button_location is not None:
            ...
            while not self._retreat_check and semi_auto and not full_auto and not self.image_tools.confirm_location("exp_gained", tries = 1) and not self.image_tools.confirm_location("no_loot",
                                                                                                                                                                                       tries = 1):
            ...
            while not self._retreat_check and not semi_auto and full_auto and not self.image_tools.confirm_location("exp_gained", tries = 1) and not self.image_tools.confirm_location("no_loot",
                                                                                                                                                                                       tries = 1):
                if self.image_tools.confirm_location("battle_concluded", tries = 1):
                    self.print_and_save(f"\n{self.printtime()} [COMBAT] Battle concluded suddenly.")
                    self.find_and_click_button("ok")
